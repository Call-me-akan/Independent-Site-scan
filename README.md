...
> 内置模板已**预设 31 个站点**（Shopify + embedded 页）。飞书/钉钉 Webhook 为空，需自行配置。
> 升级版本时，只要运行目录里有旧的 `config.yaml` 就会保留旧站点；全新目录则使用预设站点模板。
...
- **事件**：最近的新品 / 错误记录
- **飞书设置 / 钉钉设置**：配置 Webhook 并发测试消息

### 扫描与发送解耦（重要）

监控、扫描、发送三者是独立的，避免误触发群消息：

| 控件 | 作用 |
|---|---|
| 顶部**暂停/开始监控** | 控制后台 daemon 是否自动扫描（默认开启） |
| 站点**仅扫描** | 只抓取商品入库、检测新品，**不发送任何消息** |
| 站点**扫描并通知** | 扫描后立即推送新品到飞书/钉钉 |
| **📨 发送待发事件** | 手动把积压的未发送新品事件推送到群（解耦发送） |
| **🗑 忽略全部待发** | 把历史积压事件标记为已发送（不推送），适合清理首次建站噪音 |

建议首次用完「仅扫描」建基线后，点「🗑 忽略全部待发」清掉历史事件，之后只在新品出现时手动或用「扫描并通知」。
